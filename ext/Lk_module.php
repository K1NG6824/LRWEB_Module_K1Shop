...
namespace app\modules\module_page_shop_impulse_k1ng\ext;
...
class Lk_module{
	...
	public function LkLogdelete($log){
		...
		if(!preg_match('/^[0-9]{2}\_[0-9]{2}\_[0-9]{4}+$/i', $log))
			$this->message($this->Translate->get_translate_module_phrase('module_page_shop_impulse_k1ng','_Error'),'error');
		$param = ['log_name' => $log];
		...
		$this->message($this->Translate->get_translate_module_phrase('module_page_shop_impulse_k1ng','_LogDeleted'),'success');
	...
	public function LkDownloadLog($log){
		if(ini_get('zlib.output_compression'))
			...
			if(preg_match('/^[0-9]{2}\_[0-9]{2}\_[0-9]{4}+$/i', $log))
			{
				...
				foreach ($logs as $key) {
					fwrite($logFileHandle, $key['log_name'].$key['log_time'].LangValReplace($this->Translate->get_translate_module_phrase('module_page_shop_impulse_k1ng',$key['log_content']), json_decode(str_replace('[]','',$key['log_value']), true))."\r\n");
				}
	...
	public function LkGetUserData($user){
		...
		if($this->db->db_data['lk'][0]['mod'] == 1)
			...
			$userdata = $this->db->queryAll('lk', $this->db->db_data['lk'][0]['USER_ID'], $this->db->db_data['lk'][0]['DB_num'], "SELECT * FROM shop_players WHERE auth  = :auth",$param);
	...
	public function LkAddPromocode($post){
		...
		if(!preg_match('/^[A-z-0-9]{5,15}$/', $promo))
			$this->message($this->Translate->get_translate_module_phrase('module_page_shop_impulse_k1ng','_ErrorNamePromo'),'error');
		else if(empty($post['limit']))
			$this->message($this->Translate->get_translate_module_phrase('module_page_shop_impulse_k1ng','_LimitPromo'),'error');
		else if(!preg_match('/^\d+$/', $post['limit']))
			$this->message($this->Translate->get_translate_module_phrase('module_page_shop_impulse_k1ng','_LimitField'),'error');
		else if(empty($post['bonuspecent']))
			$this->message($this->Translate->get_translate_module_phrase('module_page_shop_impulse_k1ng','_EnterBonus'),'error');
		else if(!preg_match('/^[0-9\.]+$/', $post['bonuspecent']))
			$this->message($this->Translate->get_translate_module_phrase('module_page_shop_impulse_k1ng','_NotCorBonus'),'error');
		$param = ['code' => $promo];
		...
		if(!empty($expromo))
			$this->message($this->Translate->get_translate_module_phrase('module_page_shop_impulse_k1ng','_ExistPromo'),'error');
		...
		$this->message(LangValReplace($this->Translate->get_translate_module_phrase('module_page_shop_impulse_k1ng','_AddedPromo'),['namepromo'=>$promo]),'success');
	}
	...
	public function LkEditPromocode($post){
		...
		if(empty($post['editid']))
			...
			$this->message($this->Translate->get_translate_module_phrase('module_page_shop_impulse_k1ng','_EditPromoName'),'error');
		...
		$this->message(LangValReplace($this->Translate->get_translate_module_phrase('module_page_shop_impulse_k1ng','_EditedPromo'),['namepromo'=>$post['editpromo']]),'success');
	}
	...
	public function LkDeletePromocode($post){
		...
		$this->message($this->Translate->get_translate_module_phrase('module_page_shop_impulse_k1ng','_DeletedPromo'),'success');
	}
	...
	public function LkAddDiscord($post){
		...
		if(empty($post['webhoock_url_offon']))
			...
			if(empty($post['webhoock_url']))
				$this->message($this->Translate->get_translate_module_phrase('module_page_shop_impulse_k1ng','_EnterWebhoockUrl'),'error');
			$auth = 1;
		}
		...
		$this->message($this->Translate->get_translate_module_phrase('module_page_shop_impulse_k1ng','_Saved'),'success');
	}
	...
	public function LkAddGateway($post){
			...
			$this->message(LangValReplace($this->Translate->get_translate_module_phrase('module_page_shop_impulse_k1ng','_AddGateway'),['name'=>$this->name]), 'success');
	}
	...
	public function LkEditGateway($post){
		...
		$this->message(LangValReplace($this->Translate->get_translate_module_phrase('module_page_shop_impulse_k1ng','_DataChangesGateway'),['gateway'=>$this->name]), 'success');
	}
	...
	public function LkDeleteGateway($post){
		...
		$this->message($this->Translate->get_translate_module_phrase('module_page_shop_impulse_k1ng','_DeletedGateway'),'success');
	}
	...
	public function LkNotExistGateway($gateway){
		...
		$gateway = $this->db->queryAll('lk', $this->db->db_data['lk'][0]['USER_ID'], $this->db->db_data['lk'][0]['DB_num'], "SELECT * FROM lk_pay_service WHERE id = :id",$param);
		if(empty($gateway))
			$this->message($this->Translate->get_translate_module_phrase('module_page_shop_impulse_k1ng','_GetwNoExist'),'error');
	}
	...
	protected function LkExistGatewayAdd($post){
		...
		if(!empty($gateway))
			$this->message($this->name.$this->Translate->get_translate_module_phrase('module_page_shop_impulse_k1ng','_GetwExist'), 'error');
	}
		...
		switch ($gateway) {
			case 'freekassa':
				if(empty($post['shopid']))
					$this->message($this->Translate->get_translate_module_phrase('module_page_shop_impulse_k1ng','_ISID'), 'error');
				else if(empty($post['secret1']))
					$this->message($this->Translate->get_translate_module_phrase('module_page_shop_impulse_k1ng','_ISEC').' #1', 'error');
				else if(empty($post['secret2']))
					$this->message($this->Translate->get_translate_module_phrase('module_page_shop_impulse_k1ng','_ISEC').' #2', 'error');
				$this->name = 'FreeKassa';
			break;
			case 'interkassa':
				if(empty($post['shopid']))
					...
					$this->message($this->Translate->get_translate_module_phrase('module_page_shop_impulse_k1ng','_ISEC'), 'error');
				$this->name = 'InterKassa';
			break;
			case 'robokassa':
				if(empty($post['shopid']))
					$this->message($this->Translate->get_translate_module_phrase('module_page_shop_impulse_k1ng','_RSID'), 'error');
				else if(empty($post['secret1']))
					$this->message($this->Translate->get_translate_module_phrase('module_page_shop_impulse_k1ng','_IPASS').' #1', 'error');
				else if(empty($post['secret2']))
					$this->message($this->Translate->get_translate_module_phrase('module_page_shop_impulse_k1ng','_IPASS').' #2', 'error');
				$this->name = 'RoboKassa';
			break;
			case 'unitpay':
				if(empty($post['secret1']))
					$this->message($this->Translate->get_translate_module_phrase('module_page_shop_impulse_k1ng','_InPub'),'error');
				else if(empty($post['secret2']))
					$this->message($this->Translate->get_translate_module_phrase('module_page_shop_impulse_k1ng','_InSec'),'error');
				$this->name = 'UnitPay';
			break;
			case 'yandexmoney':
				if(empty($post['shopid']))
					$this->message($this->Translate->get_translate_module_phrase('module_page_shop_impulse_k1ng','_IPURSE'),'error');
				else if(empty($post['secret2']))
					$this->message($this->Translate->get_translate_module_phrase('module_page_shop_impulse_k1ng','_ISEC'),'error');
			...
			case 'paypal':
				if(empty($post['shopid']))
					$this->message($this->Translate->get_translate_module_phrase('module_page_shop_impulse_k1ng','_EnterPaypalAccount'),'error');
				$this->name = 'PayPal';
			break;
			case 'qiwi':
				if(empty($post['secret1']))
			...
			break;
			default:
				$this->message($this->Translate->get_translate_module_phrase('module_page_shop_impulse_k1ng','_NINT'), 'error');
				break;
		}
	}
	...
	public function LkDelUsers(){
		...
		if($this->db->db_data['lk'][0]['mod'] == 3)
			$this->message($this->Translate->get_translate_module_phrase('module_page_shop_impulse_k1ng','_UsersDeleteK1Lk'),'error');
		else
			$this->message($this->Translate->get_translate_module_phrase('module_page_shop_impulse_k1ng','_UsersDelete'),'success');
	}

	public function LkUpdateBalance($post){
		if( !isset( $_SESSION['user_admin'] ) || IN_LR != true )exit;
		if(!preg_match('/^STEAM_[0-9]{1,2}:[0-1]:\d+$/',$post['user']))
			$this->message($this->Translate->get_translate_module_phrase('module_page_shop_impulse_k1ng','_SteamError'),'error');
	...
	*/
	public function LkOnPayment($post){
		if(empty($post['gatewayPay']))
				$this->message($this->Translate->get_translate_module_phrase('module_page_shop_impulse_k1ng','_ChangeGateway'), 'error');
		$Gateway = $this->LkGetGatewayOn($post['gatewayPay']);
		$post['steam'] = $this->LkLoadPlayerProfile($post['steam'], 2);
		if(empty($Gateway))
			$this->message($this->Translate->get_translate_module_phrase('module_page_shop_impulse_k1ng','_GatewayOnNotEzist'), 'error');
		else if(empty($post['steam']))
			$this->message($this->Translate->get_translate_module_phrase('module_page_shop_impulse_k1ng','_EnterSteam'),'error');
		if(!preg_match('/^STEAM_[0-9]{1,2}:[0-1]:\d+$/',$post['steam']))
				$this->message($this->Translate->get_translate_module_phrase('module_page_shop_impulse_k1ng','_SteamError'),'error');
		else if(empty($post['amount']))
				$this->message($this->Translate->get_translate_module_phrase('module_page_shop_impulse_k1ng','_EnterAmount'), 'error');
	...
	protected function setPay($post){
		...
		$order = time() % 100000;
		$desc = $this->Translate->get_translate_module_phrase('module_page_shop_impulse_k1ng','_OnPayUserDesc').$post['steam'];
		...
		switch ($post['gatewayPay']) {
			...
			case 'interkassa':
				if(empty($data[0]['status']))
					$this->message(LangValReplace($this->Translate->get_translate_module_phrase('module_page_shop_impulse_k1ng','_GatwayOff'),['name'=>'InterKassa']),'error');
			...
			case 'robokassa':
				if(empty($data[0]['status']))
					$this->message(LangValReplace($this->Translate->get_translate_module_phrase('module_page_shop_impulse_k1ng','_GatwayOff'),['name'=>'RoboKassa']),'error');
			...
			case 'unitpay':
				if(empty($data[0]['status']))
					$this->message(LangValReplace($this->Translate->get_translate_module_phrase('module_page_shop_impulse_k1ng','_GatwayOff'),['name'=>'UnitPay']),'error');
			...
			case 'yandexmoney':
				if(empty($data[0]['status']))
					$this->message(LangValReplace($this->Translate->get_translate_module_phrase('module_page_shop_impulse_k1ng','_GatwayOff'),['name'=>'YandexMoney']),'error');
			...
			case 'yandexmoneycard':
				if(empty($data[0]['status']))
					$this->message(LangValReplace($this->Translate->get_translate_module_phrase('module_page_shop_impulse_k1ng','_GatwayOff'),['name'=>'YandexMoneyCard']),'error');
			...
			case 'webmoney':
				if(empty($data[0]['status']))
					$this->message(LangValReplace($this->Translate->get_translate_module_phrase('module_page_shop_impulse_k1ng','_GatwayOff'),['name'=>'WebMoney']),'error');
			...
			case 'paypal':
				if(empty($data[0]['status']))
					$this->message(LangValReplace($this->Translate->get_translate_module_phrase('module_page_shop_impulse_k1ng','_GatwayOff'),['name'=>'PayPal']),'error');
			...
			case 'qiwi':
				if(empty($data[0]['status']))
					$this->message(LangValReplace($this->Translate->get_translate_module_phrase('module_page_shop_impulse_k1ng','_GatwayOff'),['name'=>'Qiwi']),'error');
	...
	protected function checkPromo($promo,$sid){
		...
		if(empty($codeInfo))
			$this->message($this->Translate->get_translate_module_phrase('module_page_shop_impulse_k1ng','_NotFoundPromo'),'error');
		else if($codeInfo[0]['attempts'] <= 0)
			$this->message($this->Translate->get_translate_module_phrase('module_page_shop_impulse_k1ng','_NoLimitPromo'),'error');
			...
			if($userPromo)
				$this->message($this->Translate->get_translate_module_phrase('module_page_shop_impulse_k1ng','_YouUsePromo'),'error');
		}
	}
	...
	public function LkCalculatePromo($promo,$steam,$amount){
		...
		if($amount < 0.1)exit (trim(json_encode(array(
							'result' => '<div class="code_error">'.LangValReplace($this->Translate->get_translate_module_phrase('module_page_shop_impulse_k1ng','_MinAmount'),['course'=>$this->Translate->get_translate_module_phrase('module_page_shop_impulse_k1ng','_AmountCourse')]).'</div>'
						))));
		else if(!preg_match('/^STEAM_[0-9]{1,2}:[0-1]:\d+$/',$steam))
			exit (trim(json_encode(array(
							'result' => $this->Translate->get_translate_module_phrase('module_page_shop_impulse_k1ng','_SteamError')
						))));
			...
			if(empty($codeInfo))
				exit (trim(json_encode(array(
							'result' => $this->Translate->get_translate_module_phrase('module_page_shop_impulse_k1ng','_NotFoundPromo')
						))));
			else if($codeInfo[0]['attempts'] <= 0)
				exit (trim(json_encode(array(
							'result' => $this->Translate->get_translate_module_phrase('module_page_shop_impulse_k1ng','_NoLimitPromo')
						))));
			else if($codeInfo[0]['auth1']){
				...
				if($userPromo)
					exit (trim(json_encode(array(
							'result' => $this->Translate->get_translate_module_phrase('module_page_shop_impulse_k1ng','_YouUsePromo')
						))));
			}
			$bonus = ($amount/100)*$codeInfo[0]['percent'];
			$newAmount = $bonus+$amount;
			exit (trim(json_encode(array(
						'result' => LangValReplace($this->Translate->get_translate_module_phrase('module_page_shop_impulse_k1ng','_BonusPromoUse'),
