...
class Paypal extends Basefunction{
  ...
  public function PPProcessPay($post = ''){
         ...
         if (strcmp ($this->res, "VERIFIED") == 0){
          ...
          $this->LkAddLog('_NewDonat', ['gateway'=>'PayPal','order'=>$this->decod[1], 'course'=>$this->Translate->get_translate_module_phrase('module_page_shop_impulse_k1ng','_AmountCourse'), 'amount' => $this->decod[2], 'steam'=>$this->decod[3]]);
          $admins = $this->db->queryAll( 'Core', 0, 0, "SELECT * FROM lvl_web_admins WHERE flags = 'z' ");
           ...
           $this->Notifications->SendNotification( 
              $this->decod[3], 
              '_YouPay', 
              ['course'=>$this->Translate->get_translate_module_phrase('module_page_shop_impulse_k1ng','_AmountCourse'),'amount'=> $post['mc_gross'],'module_translation'=>'module_page_shop_impulse_k1ng'],
              'lk/?section=payments#p'.$this->decod[1], 
              'money'
          );
