...
class Basefunction{
	...
	public function BCheckPay($gateway){
		...
		if(empty($this->pay)){
				$this->LkAddLog('_PayNotExist', ['course'=>$this->Translate->get_translate_module_phrase('module_page_shop_impulse_k1ng', '_AmountCourse'),'numberpay' => $this->decod[1], 'steam'=>$this->decod[3],'amount'=>$this->decod[2],'gateway' =>$gateway]);
					return false;
		}else return true;
	}
	...
	public function BCheckPromo($gateway){
		...
		if(empty($promoCode)){
			...
			$this->LkAddLog('_SetPromo',['course'=>$this->Translate->get_translate_module_phrase('module_page_shop_impulse_k1ng', '_AmountCourse'),'numberpay' => $this->decod[1], 'promocode'=>$this->pay[0]['pay_promo'],'amount'=>$this->bonus,'gateway' =>$gateway]);
		}
	}
				...
				$json = json_encode([
					...
					[
				        [	
				            ...
				            "thumbnail" =>
				            [
				                "url" => 'http:'.$this->General->arr_general['site']."app/modules/module_page_shop_impulse_k1ng/assets/gateways_discord/".mb_strtolower($kassa).".png",
				            ],
				            "footer"=>
				            [
				            ...
				            [
				               	[
				                    "name" => $this->Translate->get_translate_module_phrase('module_page_shop_impulse_k1ng', '_Replenishment'),
				                    "value" => $this->decod[3],
				                    "inline" => true
				                ],
				                [
				                    "name" => $this->Translate->get_translate_module_phrase('module_page_shop_impulse_k1ng', '_Amount'),
				                    "value" => $this->decod[2],
				                    "inline" => false
				                ]
				            ]
	...
	public function BUpdateBalancePlayer($steam,$summ){
		preg_match('/:[0-9]{1}:\d+/i', $steam, $auth);
		 ...
		 $params = [
			...
			if($this->db->db_data['lk'][0]['mod'] == 1)
				$this->db->query('lk', $this->db->db_data['lk'][0]['USER_ID'], $this->db->db_data['lk'][0]['DB_num'], "UPDATE lk SET money = money + :money, all_money = all_money + :all_money WHERE auth LIKE :auth", $params);
			else if($this->db->db_data['lk'][0]['mod'] == 2)
