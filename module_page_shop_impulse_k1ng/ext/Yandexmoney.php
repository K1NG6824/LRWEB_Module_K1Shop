...
class Yandexmoney extends Basefunction{
	...
	public function YMProcessPay($post){
		$BCheckPay = $this->BCheckPay('YandexMoney');
		 ...
		 $this->LkAddLog('_NewDonat', ['gateway'=>'YandexMoney','order'=>$this->decod[1], 'course'=>$this->Translate->get_translate_module_phrase('module_page_shop_impulse_k1ng','_AmountCourse'), 'amount' => $this->decod[2], 'steam'=>$this->decod[3]]);
		 ...
		 $this->Notifications->SendNotification( 
			 	$this->decod[3], 
			 	'_YouPay', 
			 	['course'=>$this->Translate->get_translate_module_phrase('module_page_shop_impulse_k1ng','_AmountCourse'),'amount'=> $post['withdraw_amount'],'module_translation'=>'module_page_shop_impulse_k1ng'],
			 	'lk/?section=payments#p'.$this->decod[1], 
			 	'money'
		);
		 die('YES');
