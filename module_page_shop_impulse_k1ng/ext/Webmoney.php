...
use app\modules\module_page_shop_impulse_k1ng\ext\Basefunction;
...
class Webmoney extends Basefunction{
	...
	public function WBProcessPay($post){
		 ...
		 $this->LkAddLog('_NewDonat', ['gateway'=>'WebMoney','order'=>$this->decod[1], 'course'=>$this->Translate->get_translate_module_phrase('module_page_shop_impulse_k1ng','_AmountCourse'), 'amount' => $this->decod[2], 'steam'=>$this->decod[3]]);
		 $admins = $this->db->queryAll( 'Core', 0, 0, "SELECT * FROM lvl_web_admins WHERE flags = 'z' ");
		 foreach( $admins as $key ){
			 $this->Notifications->SendNotification(
			 		 con_steam64to32($key['steamid']), 
			 		 '_GetDonat', 
			 		 ['course'=>$this->Translate->get_translate_module_phrase('module_page_shop_impulse_k1ng','_AmountCourse'),'amount'=> $post['AMOUNT'],'module_translation'=>'module_page_shop_impulse_k1ng'],
		 ...
		 $this->Notifications->SendNotification( 
			 	$this->decod[3], 
			 	'_YouPay', 
			 	['course'=>$this->Translate->get_translate_module_phrase('module_page_shop_impulse_k1ng','_AmountCourse'),'amount'=> $post['LMI_PAYMENT_AMOUNT'],'module_translation'=>'module_page_shop_impulse_k1ng'],
			 	'lk/?section=payments#p'.$this->decod[1], 
			 	'money'
		);
		die('YES');
